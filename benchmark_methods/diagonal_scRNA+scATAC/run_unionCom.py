...
# %%
def unionCom_module(input_path,
                    output_path,
                    config):
    ...
    integrated_data = uc.fit_transform(dataset=[rna_hvg.X.A, atac.X.A])

    # latent
    df = pd.DataFrame(integrated_data[0])
    df.index = rna.obs_names
    df.to_csv(os.path.join(output_path, config["output_prefix"] + "-unionCom-RNA-latent.csv"))

    df = pd.DataFrame(integrated_data[1])
    df.index = atac.obs_names
    df.to_csv(os.path.join(output_path, config["output_prefix"] + "-unionCom-ATAC-latent.csv"))
    ...
    gpu_memory.to_csv(os.path.join(output_path, config['output_prefix'] + '-unionCom-gpu_memory.csv'), header=["gpu_memory"])


# %%
unionCom_module(input_path = sys.argv[1], 
...
# output_path = "/home/wsg/BM/results/task/diagonal_scRNA+scATAC/accuracy/BMMC/p10_GAM/rep_1/run_unionCom"
# config = json.load(open("/home/wsg/BM/data/BMMC/RNA+ATAC/p10/p10.json"))

# %%
# unionCom_module(input_path,
#               output_path,
#               config)
