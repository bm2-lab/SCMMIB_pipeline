...
dataset <- unlist(fromJSON(file = "/home/wsg/BM/data/COVID19/RNA+ADT/RawData/RawData.json"))

# %%
COVID19_RNA_raw <- read_h5ad(file = paste0(input_path, "/COVID19-CITE_seq-raw-RNA-counts.h5ad"))

# %%
process = "raw"
RNA_subset_counts <- t(COVID19_RNA_raw$X)
dim(RNA_subset_counts)

# %%
RNA_subset_counts <- as(RNA_subset_counts, "CsparseMatrix")

# %%
RNA_subset_counts

# %%
rna_subset_meta <- COVID19_RNA_raw$obs
rna_subset_meta["barcode"] <- rownames(rna_subset_meta)
head(rna_subset_meta)
...
# %%
COVID19_ADT_raw <- read_h5ad(file = paste0(input_path, "/COVID19-CITE_seq-raw-ADT-counts.h5ad"))

# %%
ADT_subset_counts <- t(COVID19_ADT_raw$X)
dim(ADT_subset_counts)

# %%
ADT_subset_counts <- as(ADT_subset_counts, "CsparseMatrix")

# %%
ADT_subset_counts

# %%
adt_subset_meta <- COVID19_ADT_raw$obs
head(adt_subset_meta)

# %%
process = "raw"

# save raw adt to mtx
data_path <- here(output_path, "COVID19-CITE_seq-raw-ADT-counts.mtx")
...
saveRDS(ADT_subset_counts, 
        file = here(output_path, "COVID19-CITE_seq-raw-ADT-counts.rds"))
...
SaveH5Seurat(ADT_subset, overwrite = TRUE, 
             filename = here(output_path, 
                                   "COVID19-CITE_seq-raw-ADT-counts.h5Seurat"))
...
# %%
input_path <- "/home/wsg/BM/data/COVID19/RNA+ADT/RawData"
output_path <- "/home/wsg/BM/data/COVID19/RNA+ADT/c500k"
dataset <- unlist(fromJSON(file = "/home/wsg/BM/data/COVID19/RNA+ADT/RawData/RawData.json"))
...
# RNA_counts <- t(COVID19_RNA_raw$X)
# dim(RNA_counts)

# metadata <- COVID19_RNA_raw$obs
# metadata["barcode"] <- rownames(metadata)
# metadata["cell_type"] <- metadata$initial_clustering

# %%
# random sample Ncells of data
rna_subset_meta <- metadata %>% slice_sample(n=Ncell)

table(rna_subset_meta$Sex)
table(rna_subset_meta$cell_type)
...
# save raw rna to mtx
data_path <- here(output_path,
                 paste("COVID19-CITE_seq", process,
                       "RNA", "counts.mtx", sep = "-"))
write10xCounts(x = RNA_subset_counts, path = data_path, version = "3")
write_csv(rna_subset_meta, here(output_path, "metadata.csv"))
...
# Convert h5Seurat to h5ad
setwd(output_path)
Convert(paste("COVID19-CITE_seq", process, "RNA", "counts.h5Seurat", sep = "-"), 
        dest = "h5ad")

# %%
table(metadata[,c("Site", "sample_id")])

# %%
# COVID19_ADT_raw <- read_h5ad(file = paste0(input_path, "/COVID19-CITE_seq-raw-ADT-counts.h5ad"))

# ADT_counts <- t(COVID19_ADT_raw$X)
# dim(ADT_counts)

# metadata <- COVID19_ADT_raw$obs
# metadata["barcode"] <- rownames(metadata)
# metadata["cell_type"] <- metadata$initial_clustering
...
# save raw adt to h5ad
setwd(output_path)
Convert(paste("COVID19-CITE_seq", process, "ADT", "counts.h5Seurat", sep = "-"), 
        dest = "h5ad")
...
output_path <- "/home/wsg/BM/data/BMMC/RNA+ATAC/c5k_c5k"
...
meta_ref <- metadata_others %>% slice_sample(n=5000)

meta_qry$data_size <- "c5k_1"
meta_ref$data_size <- "c5k_2"
